...
공개 비디오 데이터셋을 활용한 시뮬레이션 실험에서, 문헌 기반 혼합 정책(frame_action_single_path)은 기준 정책(heuristic_baseline) 대비 late_frame_ratio를 0.3267에서 0.2071로 약 36.6% 개선하였다. 다중경로 정책(frame_action_multipath)도 0.2431로 기준 정책 대비 25.6%의 개선을 보였다. 이 결과는 프레임 단위 행동공간 설계가 비디오 전송 효율 향상에 유효함을 실증하며, 향후 ML/RL 기반 중요도 판단 모델로의 확장 가능성을 시사한다.
